...
export default function SGPAToCGPAClient() {
  ...
  const handleScanComplete = (extracted: ExtractedSubject[]) => {
    const newSemesters = extracted.map((s, index) => ({
      id: Math.random().toString(),
      sgpa: s.grade, // In SGPA mode, the 'grade' field contains the SGPA value
      credits: s.credits,
    }));
  ...
  return (
    <CalculatorCard title="SGPA to CGPA Multi-Semester Tool">
      <div className="space-y-4">
        <ResultScanner university="General" onScanComplete={handleScanComplete} />
        
        <div className="grid grid-cols-12 gap-4 mb-2 text-xs font-semibold text-slate-400 uppercase tracking-wider px-2">
          <div className="col-span-1">#</div>
          <div className="col-span-5 md:col-span-6">SGPA</div>
          ...
          <div key={sem.id} className="grid grid-cols-12 gap-4 items-center">
            ...
            <div className="col-span-5 md:col-span-6">
              <input
                ...
                className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-primary-500 outline-none transition-all placeholder:text-slate-300"
              />
            </div>
        ...
        <div className="pt-4 flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          {cgpa !== null && (
            <div className="w-full md:w-auto bg-primary-600 text-white px-8 py-4 rounded-2xl shadow-lg flex flex-col items-center justify-center">
              <span className="text-xs font-semibold uppercase opacity-80 mb-1">Cumulative CGPA</span>
